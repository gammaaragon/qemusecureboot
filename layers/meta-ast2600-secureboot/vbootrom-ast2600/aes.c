/* SPDX-License-Identifier: GPL-2.0-or-later */
/*
 * AES encryption, generalized for AES-128/192/256 from U-Boot's
 * lib/aes.c (SPDX-License-Identifier: GPL-2.0+; original AES core
 * public domain, Karl Malbrain -- see aes.h's own comment for the full
 * story and why only the encrypt direction is ported). The S-box
 * tables and the shift_rows/mix_sub_columns/add_round_key/
 * aes_expand_key algorithms are unmodified from that file -- only the
 * AES_KEYCOLS/AES_ROUNDS enum constants (fixed to AES-128 there) became
 * runtime parameters here. AES_STATECOLS (the block size in 32-bit
 * words) is NOT one of those -- it's always 4 regardless of key size
 * (the AES block is always 128 bits), so it stays a literal, same as
 * the original.
 */
#include "aes.h"
#include "libc_min.h"

/* forward s-box */
static const uint8_t sbox[256] = {
    
};

/* combined Xtimes2[Sbox[]] */
static const uint8_t x2_sbox[256] = {
    
};

/* combined Xtimes3[Sbox[]] */
static const uint8_t x3_sbox[256] = {
    
};

static void shift_rows(uint8_t *state)
{
    uint8_t tmp;

    /* just substitute row 0 */
    state[0] = sbox[state[0]];
    state[4] = sbox[state[4]];
    state[8] = sbox[state[8]];
    state[12] = sbox[state[12]];

    /* rotate row 1 */
    tmp = sbox[state[1]];
    state[1] = sbox[state[5]];
    state[5] = sbox[state[9]];
    state[9] = sbox[state[13]];
    state[13] = tmp;

    /* rotate row 2 */
    tmp = sbox[state[2]];
    state[2] = sbox[state[10]];
    state[10] = tmp;
    tmp = sbox[state[6]];
    state[6] = sbox[state[14]];
    state[14] = tmp;

    /* rotate row 3 */
    tmp = sbox[state[15]];
    state[15] = sbox[state[11]];
    state[11] = sbox[state[7]];
    state[7] = sbox[state[3]];
    state[3] = tmp;
}

/* recombine and mix each row in a column */
static void mix_sub_columns(uint8_t *state)
{
    uint8_t tmp[16];

    /* mixing column 0 */
    tmp[0] = x2_sbox[state[0]] ^ x3_sbox[state[5]] ^
             sbox[state[10]] ^ sbox[state[15]];
    tmp[1] = sbox[state[0]] ^ x2_sbox[state[5]] ^
             x3_sbox[state[10]] ^ sbox[state[15]];
    tmp[2] = sbox[state[0]] ^ sbox[state[5]] ^
             x2_sbox[state[10]] ^ x3_sbox[state[15]];
    tmp[3] = x3_sbox[state[0]] ^ sbox[state[5]] ^
             sbox[state[10]] ^ x2_sbox[state[15]];

    /* mixing column 1 */
    tmp[4] = x2_sbox[state[4]] ^ x3_sbox[state[9]] ^
             sbox[state[14]] ^ sbox[state[3]];
    tmp[5] = sbox[state[4]] ^ x2_sbox[state[9]] ^
             x3_sbox[state[14]] ^ sbox[state[3]];
    tmp[6] = sbox[state[4]] ^ sbox[state[9]] ^
             x2_sbox[state[14]] ^ x3_sbox[state[3]];
    tmp[7] = x3_sbox[state[4]] ^ sbox[state[9]] ^
             sbox[state[14]] ^ x2_sbox[state[3]];

    /* mixing column 2 */
    tmp[8] = x2_sbox[state[8]] ^ x3_sbox[state[13]] ^
             sbox[state[2]] ^ sbox[state[7]];
    tmp[9] = sbox[state[8]] ^ x2_sbox[state[13]] ^
             x3_sbox[state[2]] ^ sbox[state[7]];
    tmp[10] = sbox[state[8]] ^ sbox[state[13]] ^
              x2_sbox[state[2]] ^ x3_sbox[state[7]];
    tmp[11] = x3_sbox[state[8]] ^ sbox[state[13]] ^
              sbox[state[2]] ^ x2_sbox[state[7]];

    /* mixing column 3 */
    tmp[12] = x2_sbox[state[12]] ^ x3_sbox[state[1]] ^
              sbox[state[6]] ^ sbox[state[11]];
    tmp[13] = sbox[state[12]] ^ x2_sbox[state[1]] ^
              x3_sbox[state[6]] ^ sbox[state[11]];
    tmp[14] = sbox[state[12]] ^ sbox[state[1]] ^
              x2_sbox[state[6]] ^ x3_sbox[state[11]];
    tmp[15] = x3_sbox[state[12]] ^ sbox[state[1]] ^
              sbox[state[6]] ^ x2_sbox[state[11]];

    memcpy(state, tmp, sizeof(tmp));
}

/* encrypt/decrypt columns of the key
 * n.b. you can replace this with byte-wise xor if you wish. */
static void add_round_key(uint32_t *state, uint32_t *key)
{
    int idx;

    for (idx = 0; idx < 4; idx++) {
        state[idx] ^= key[idx];
    }
}

static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

void aes_expand_key(const uint8_t *key, unsigned int key_words,
                    uint8_t *expkey, unsigned int *rounds_out)
{
    uint8_t tmp0, tmp1, tmp2, tmp3, tmp4;
    unsigned int idx;
    unsigned int rounds = key_words + 6;
    unsigned int total_words = 4 * (rounds + 1);
    /*
     * col/rc track idx % key_words / idx / key_words incrementally
     * instead of computing them with a runtime division each
     * iteration -- key_words is a small variable (4/6/8) here, not a
     * compile-time constant, so `idx % key_words` would otherwise pull
     * in libgcc's software __aeabi_uidivmod, which (in this
     * cross-toolchain's -nostdlib freestanding build) drags in a
     * div-by-zero handler that itself references libc's raise() --
     * an unresolvable link error for a target with no libc at all.
     * col starts at 0 and rc at 1 to match idx's real mod/div values
     * at the loop's first iteration (idx == key_words).
     */
    unsigned int col = 0;
    unsigned int rc = 1;

    *rounds_out = rounds;

    memcpy(expkey, key, key_words * 4);

    for (idx = key_words; idx < total_words; idx++) {
        tmp0 = expkey[4 * idx - 4];
        tmp1 = expkey[4 * idx - 3];
        tmp2 = expkey[4 * idx - 2];
        tmp3 = expkey[4 * idx - 1];
        if (col == 0) {
            tmp4 = tmp3;
            tmp3 = sbox[tmp0];
            tmp0 = sbox[tmp1] ^ rcon[rc];
            tmp1 = sbox[tmp2];
            tmp2 = sbox[tmp4];
        } else if ((key_words > 6) && (col == 4)) {
            tmp0 = sbox[tmp0];
            tmp1 = sbox[tmp1];
            tmp2 = sbox[tmp2];
            tmp3 = sbox[tmp3];
        }

        expkey[4 * idx + 0] = expkey[4 * idx - 4 * key_words + 0] ^ tmp0;
        expkey[4 * idx + 1] = expkey[4 * idx - 4 * key_words + 1] ^ tmp1;
        expkey[4 * idx + 2] = expkey[4 * idx - 4 * key_words + 2] ^ tmp2;
        expkey[4 * idx + 3] = expkey[4 * idx - 4 * key_words + 3] ^ tmp3;

        col++;
        if (col == key_words) {
            col = 0;
            rc++;
        }
    }
}

void aes_encrypt_block(const uint8_t *in, const uint8_t *expkey,
                       unsigned int rounds, uint8_t *out)
{
    uint8_t state[16];
    unsigned int round;

    memcpy(state, in, 16);
    add_round_key((uint32_t *)state, (uint32_t *)(uintptr_t)expkey);

    for (round = 1; round < rounds + 1; round++) {
        if (round < rounds) {
            mix_sub_columns(state);
        } else {
            shift_rows(state);
        }

        add_round_key((uint32_t *)state,
                      (uint32_t *)(uintptr_t)(expkey + round * 16));
    }

    memcpy(out, state, sizeof(state));
}
