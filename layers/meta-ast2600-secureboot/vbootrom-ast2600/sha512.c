...
/*
 * FIPS-180-2 compliant SHA-512 (and SHA-384, added below), ported
 * near-verbatim from U-Boot's lib/sha512.c (SPDX-License-Identifier:
 * GPL-2.0+; original copyright Jean-Luc Cooke, Andrew McDonald, Kyle
 * McMartin, Reuben Dowle -- see that file for full attribution). Only
 * change from the original: dropped the watchdog-chunking wrapper
 * (sha512_csum_wd -- no watchdog concerns for a single-shot boot-ROM
 * verify), and swapped <common.h>/<linux/string.h> for this project's
 * own libc_min.h. The compression function (sha512_transform) and the
...
static void sha512_block_fn(sha512_context *sst, const uint8_t *src, int blocks)
{
    while (blocks--) {
...
static void sha512_base_do_update(sha512_context *sctx, const uint8_t *data,
                                  unsigned int len)
{
...
{
    const int bit_offset = SHA512_BLOCK_SIZE - (int)sizeof(uint64_t[2]);
    uint64_t *bits = (uint64_t *)(sctx->buf + bit_offset);
    unsigned int partial = sctx->count[0] % SHA512_BLOCK_SIZE;

    sctx->buf[partial++] = 0x80;
    if ((int)partial > bit_offset) {
...
void sha512_finish(sha512_context *ctx, uint8_t digest[SHA512_SUM_LEN])
{
    ...
    for (i = 0; i < SHA512_SUM_LEN / (int)sizeof(uint64_t); i++) {
...
{
    /* SHA-384 is SHA-512 with a different initial state (sha384_starts)
     * and truncated output -- run the identical finalize path into a
     * full 64-byte buffer, then keep only the first 48 bytes. */
    int i;
    uint8_t full[SHA512_SUM_LEN];

    sha512_base_do_finalize(ctx);
    for (i = 0; i < SHA512_SUM_LEN / (int)sizeof(uint64_t); i++) {
        PUT_UINT64_BE(ctx->state[i], full, i * 8);
    }
    memcpy(digest, full, SHA384_SUM_LEN);
}
